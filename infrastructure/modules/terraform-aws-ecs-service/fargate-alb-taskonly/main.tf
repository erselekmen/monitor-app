...
resource "aws_lb_listener_rule" "lb_rule" {
  ...
  condition {
    dynamic "http_header" {
      for_each = var.http_header_enabled ? [1] : []
      content {
        http_header_name = var.http_header_name
        values           = var.http_headers
      }
    }
  }
...
resource "aws_lb_listener_rule" "path_lb_rule" {
  ...
  condition {
    dynamic "http_header" {
      for_each = var.path_http_header_enabled ? [1] : []
      content {
        http_header_name = var.path_http_header_name
        values           = var.path_http_headers
      }
    }
  }
...
resource "aws_lb_target_group" "lb_tg_internal_additional" {
  count                = var.internal_additional_listener_enabled ? 1 : 0
  name                 = "${var.target_group_name}-additional"
  port                 = var.app_container_additional_port
  protocol             = "HTTPS"
  protocol_version     = var.tg_protocol_version_additional
  target_type          = "ip"
  vpc_id               = var.vpc_id
  tags                 = var.tags
  deregistration_delay = var.deregistration_delay
  ...
  health_check {
    matcher             = var.health_check_matcher_additional
    protocol            = var.health_check_protocol
    path                = var.health_check_path_additional
    interval            = var.health_check_interval
    unhealthy_threshold = var.health_check_unhealthy_threshold
  }
  stickiness {
...
resource "aws_lb_listener_rule" "lb_rule_internal_additional" {
  count        = var.internal_additional_listener_enabled ? 1 : 0
  listener_arn = var.aws_lb_internal_additional_listener_arn
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.lb_tg_internal_additional[0].arn
  }
  condition {
    host_header {
      values = var.internal_additional_host_headers
    }
  }
}
    ...
    content {
      ...
      efs_volume_configuration {
        ...
        dynamic "authorization_config" {
          for_each = lookup(volume.value, "authorization_config", "") != "" ? [1] : []
          content {
            access_point_id = lookup(volume.value, "authorization_config", null)
...
resource "aws_ecs_service" "service" {
  ...
  dynamic "load_balancer" {
    for_each = var.internal_additional_listener_enabled ? aws_lb_target_group.lb_tg_internal_additional.*.arn : []

    content {
      target_group_arn = load_balancer.value
      container_name   = "${var.namespace}-${var.environment}-${var.app_container_name}"
      container_port   = var.app_container_additional_port
    }
  }
